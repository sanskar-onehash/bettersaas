...
def get_all_stock_sites():
    stock_sites = frappe.db.get_list(
        "SaaS Stock Sites", fields=["name", "subdomain"], ignore_permissions=True
    )
    return stock_sites
...
def refresh_stock_sites(*args, **kwargs):
    ...
    if (method not in queued_jobs[frappe.local.site]) and (
        len(current_stock) < config.stock_sites_count
    ):
        ...
        for _ in range(number_of_sites_to_stock):
            ...
            commands.append(
                "bench --site {} install-app payments".format(subdomain + "." + domain)
            )
            commands.append(
            ...
            commands.append("bench --site {} migrate".format(subdomain + "." + domain))
